...
> 版本: v1.0 | 更新日期: 2026-02-07
> 画布尺寸: 960 x 540 像素
> 美术风格: 梦幻西游界面布局 + 曹操传像素精美度（暗色系像素风）
...
画布 960x540，**环形/弧形排列**（全员混战，无阵营区分）。

> **设计决策**：本游戏为全员混战（Free-for-all），每个角色独立，互相可攻击。
> 不使用梦幻西游式左右对阵，因为无法合理分配3人、5人、7人等奇数场景。
> 改用弧形排列，所有角色围成弧形，视觉上平等，无阵营暗示。
...
│                                                                    │
│ ┌──────────────────────┐ ┌─────────────────────────────────────┐  │
│ │     战斗日志 (160px)  │ │          动作面板 (160px)            │  │
│ │                      │ │  ┌──────┐ ┌──────┐ ┌──────┐        │  │
│ │  骑士 对 装甲战士     │ │  │ 普攻  │ │ 防御  │ │ 逃跑  │        │  │
│ │  使用 升龙击          │ │  ├──────┤ ├──────┤ ├──────┤        │  │
│ │  造成 35.00 伤害      │ │  │技能1 │ │技能2 │ │技能3 │        │  │
│ │                      │ │  ├──────┤ ├──────┤ ├──────┤        │  │
│ │                      │ │  │技能4 │ │技能5 │ │技能6 │        │  │
│ └──────────────────────┘ │  └──────┘ └──────┘ └──────┘        │  │
│                          └─────────────────────────────────────┘  │
...
| 用途 | 颜色代码 | 色名 | 说明 |
|------|---------|------|------|
| 主背景 | `#1A1A2E` | 深靛蓝 | 战斗场景背景底色 |
| 副背景 | `#16213E` | 暗海军蓝 | 面板/对话框背景 |
| 面板色 | `#0F3460` | 深宝蓝 | 按钮、卡片的深色底 |
| 高亮色 | `#E94560` | 胭脂红 | 强调、警告、伤害 |
| 金色 | `#FFD700` | 金色 | 玩家标识、标题、重要信息 |

#### 战斗场景背景

```
天空渐变: #1A1A2E (顶) → #16213E (中) → #0F3460 (底)
地面: #2D2D44 (暗紫灰，像素风地砖纹理)
地面高亮线: #3D3D55 (网格线，微弱可见)
```
...
游戏以 960x540 为基准设计（16:9），不做自适应布局，而是采用等比缩放策略：
- 浏览器窗口大于 960x540 → 画布居中，保持原始分辨率（像素风锐利）
- 浏览器窗口小于 960x540 → 等比缩小画布适配，保持 16:9 比例
