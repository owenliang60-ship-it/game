...
- **HP消耗时机**: HP消耗型技能（如冲锋）的HP消耗在技能**发动时立即扣除**，随后执行攻击效果。若HP消耗导致自身HP ≤ 0，技能仍然发动，攻击效果正常生效，然后攻击者死亡
- **平局判定**: 若回合结束时存活角色数为0（如最后两名角色互杀），判定为**平局**，无获胜者。结算画面显示"平局"
...
- 消耗 60 怒气
- **在动作结算阶段（RESOLVE）最先结算**，无视敏捷排序。装甲战士选择反震后，在所有其他角色行动之前，反震状态立即生效
...
| 2026-02-08 | v0.9.1 | **审计修复**：(1)反震时序改为"RESOLVE阶段最先结算"（修正原"回合开始生效"的逻辑矛盾）；(2)新增HP消耗时机规则（发动时立即扣除）；(3)新增平局判定规则（0存活者=平局）；(4)GAME_DESIGN.md归档降级 |
